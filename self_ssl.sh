...
#fi
echo -e "$CYAN Creating initial SSL certificate...$COL_RESET"
...
if [ ! -f $STORAGE_ROOT/ssl/ssl_private_key.pem ]; then
	...
	(umask 077; sudo openssl genrsa -out $STORAGE_ROOT/ssl/ssl_private_key.pem 2048)
fi
...
if [ ! -f $STORAGE_ROOT/ssl/ssl_certificate.pem ]; then
	...
	sudo openssl req -new -key $STORAGE_ROOT/ssl/ssl_private_key.pem -out $CSR -sha256 -subj "/CN=$PRIMARY_HOSTNAME"
