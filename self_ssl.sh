...
hide_output apt_install openssl

# Create a directory to store TLS-related things like "SSL" certificates.

hide_output sudo mkdir -p $STORAGE_ROOT/ssl
...
if [ ! -f $STORAGE_ROOT/ssl/ssl_certificate.pem ]; then
    ...
    hide_output sudo rm -f $CSR

	# Symlink the certificate into the system certificate path, so system services
	# can find it.
    hide_output sudo ln -s $CERT $STORAGE_ROOT/ssl/ssl_certificate.pem
fi
