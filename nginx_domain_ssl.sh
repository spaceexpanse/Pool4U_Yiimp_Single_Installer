...
echo -e "$CYAN Generating Certbot Request for ${DomainName} ...$COL_RESET"
hide_output sudo mkdir -p /var/www/_letsencrypt
hide_output sudo chown www-data /var/www/_letsencrypt
hide_output hide_output sudo certbot certonly --webroot -d "${DomainName}" --register-unsafely-without-email -w /var/www/_letsencrypt -n --agree-tos --force-renewal
...
if sudo [ -f /etc/letsencrypt/live/"$DomainName"/fullchain.pem ]; then
    ...
    hide_output sudo mkdir -p /etc/letsencrypt/renewal-hooks/post/
    echo '#!/bin/bash\nnginx -t && systemctl reload nginx' | sudo -E tee /etc/letsencrypt/renewal-hooks/post/nginx-reload.sh >/dev/null 2>&1
    hide_output sudo chmod a+x /etc/letsencrypt/renewal-hooks/post/nginx-reload.sh
    # Remove the '"${DomainName}"'.conf that had the self signed SSL and replace with the new file.
    hide_output sudo rm /etc/nginx/sites-available/${DomainName}.conf
    # I am SSL Man!
echo '######################################
